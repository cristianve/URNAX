...
import Domain.{Direction, Person}

import io.circe.{Decoder, HCursor, Json}

object Person {
  // here we are actually casting the return value to Decode
  implicit val decoder: Decoder[Person] = (hCursor: HCursor) =>
    for {
    ...
    } yield Domain.Person(id, name, surname, nif, new Direction(city,state,postalCode), hasVoted)
}
